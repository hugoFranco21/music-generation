...
def compute_composition_stats(rl_tuner,
                              ...
                              key=None,
                              tonic_note=helpers.C_MAJOR_TONIC):
...
def get_stat_dict_string(stat_dict, print_interval_stats=True):
    ...
    if print_interval_stats:
        ...
        return_str += str(float(stat_dict['num_octave_jumps']
                                ) / tot_comps) + '\n'
...
def compose_and_evaluate_piece(rl_tuner,
    ...
    for _ in range(composition_length):
        ...
        stat_dict = add_interval_stat(
            rl_tuner, new_observation, stat_dict, key=key)
        ...
        stat_dict = add_repeated_motif_stat(
            rl_tuner, new_observation, stat_dict)
    ...
    for lag in [1, 2, 3]:
        stat_dict['autocorrelation' + str(lag)].append(
            helpers.autocorrelate(rl_tuner.composition, lag))
...
def add_interval_stat(rl_tuner, action, stat_dict, key=None):
    ...
    elif interval == helpers.REST_INTERVAL_AFTER_THIRD_OR_FIFTH:
        stat_dict['num_special_rest_intervals'] += 1
    elif interval > helpers.OCTAVE:
        stat_dict['num_octave_jumps'] += 1
    elif interval == (helpers.IN_KEY_FIFTH or
                    interval == helpers.IN_KEY_THIRD):
        stat_dict['num_in_key_preferred_intervals'] += 1
    elif interval == helpers.FIFTH:
        stat_dict['num_fifths'] += 1
    elif interval == helpers.THIRD:
        stat_dict['num_thirds'] += 1
    elif interval == helpers.SIXTH:
        stat_dict['num_sixths'] += 1
    elif interval == helpers.SECOND:
        stat_dict['num_seconds'] += 1
    elif interval == helpers.FOURTH:
        stat_dict['num_fourths'] += 1
    elif interval == helpers.SEVENTH:
