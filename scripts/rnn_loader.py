...
class RNNLoader(object):
    ...
    def __init__(self, graph, scope, dir, file=None,
                midi_primer=None, training_file_list=None):
        ...
        self.variable_names = helpers.get_variable_names(self.graph,
                                                            self.scope)
    ...
    def build_graph(self):
        ...
        tf.logging.info(
            'Initializing melody RNN graph for scope %s', self.scope)
    ...
    def load_primer(self):
        ...
        if not os.path.exists(self.midi_primer):
            tf.logging.warn(
                'ERROR! No such primer file exists! %s', self.midi_primer)
            return
    ...
    def prime_model(self):
        ...
        with self.graph.as_default():
            tf.logging.debug(
                'Priming the model with MIDI file %s', self.midi_primer)
            ...
            primer_input_batch = np.tile(
                [primer_input], (self.batch_size, 1, 1))
    ...
    def get_note_from_softmax(self, softmax):
        ...
        note_enc = rl_tuner_ops.make_onehot(
            [note_idx], rl_tuner_ops.NUM_CLASSES)
